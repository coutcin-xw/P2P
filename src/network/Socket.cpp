...
public:
  explicit UDPSocket(bool isIPv6) {
    ...
    if (isIPv6) {
      int opt = 0;
      if (setsockopt(sockfd_, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&opt,
                     sizeof(opt)) != 0) {
        // 非致命错误，记录日志但继续运行
      }
    }
  ...
  ssize_t sendTo(const void *buf, size_t len, const IIPAddress &ipAddress,
                 const Port &port) override {
    sockaddr_storage addr;
    memset(&addr, 0, sizeof(addr));
  ...
  ssize_t recvFrom(void *buf, size_t len,
                   ...
                   std::unique_ptr<Port> &srcPort) override {
    sockaddr_storage addr;
  ...
  std::unique_ptr<IIPAddress> getLocalIPAddress() const noexcept override {
    ...
    if (getsockname(sockfd_, reinterpret_cast<sockaddr *>(&addr), &addrlen) !=
        0) {
      return nullptr;
    }
...
public:
  ...
  bool listen(int backlog) override { return ::listen(sockfd_, backlog) == 0; }

  std::unique_ptr<ITCPSocket>
  accept(std::unique_ptr<IIPAddress> &clientIpAddress,
         std::unique_ptr<Port> &clientPort) override {
    sockaddr_storage addr;
    socklen_t addrlen = sizeof(addr);

    SOCKET_HANDLE clientSockfd =
        ::accept(sockfd_, reinterpret_cast<sockaddr *>(&addr), &addrlen);

    if (clientSockfd == SOCKET_ERROR_VAL) {
      throw std::system_error(errno, std::generic_category(),
                              "accept() failed");
    }
  ...
  bool connect(const IIPAddress &ipAddress, const Port &port) override {
    ...
    return ::connect(sockfd_, reinterpret_cast<sockaddr *>(&addr),
                     sizeof(addr)) == 0;
  }
...
private:
  explicit TCPSocket(SOCKET_HANDLE sockfd) : sockfd_(sockfd) {}
  SOCKET_HANDLE sockfd_ = -1; // socket 文件描述符
};
